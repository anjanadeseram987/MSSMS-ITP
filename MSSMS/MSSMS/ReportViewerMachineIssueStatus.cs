...
namespace MSSMS
{
    public partial class ReportViewerMachineIssueStatus : Form
    {
        ...
        public ReportViewerMachineIssueStatus(ReportType reportType, DateTime startDateTime, DateTime endDateTime)
        {
            InitializeComponent();
        ...
        private void ReportViewerMachineIssueStatus_Load(object sender, EventArgs e)
        {
            NotificationManager.hideInAppNotification(panelInAppNotifications);
        }
        ...
        private void generateMonthlyReport()
        {
            ...
            try
            {
                machines = reportsDBHandler.getMonthlyMachineIssuesStatusReport(this.statDateTime);
                dataGridReport.Rows.Clear();

                //binding
                foreach (Machine machine in machines)
                {
                    dataGridReport.Rows.Add(machine.machineId, machine.serialNumber, machine.name, machine.loc_name,machine.total_machine_issues, machine.critical_machine_issues, machine.resolved_machine_issues, (Math.Round(((((Decimal) machine.resolved_machine_issues) / ((Decimal) machine.total_machine_issues)) * 100),2)).ToString() + "%" );
                }
            }
